...
def format_html_content(html: str) -> str:
    """Add proper line breaks and indentation between HTML block elements."""
    # Strip all em dashes — the user doesn't want them
    html = html.replace("\u2014", "-")
    html = html.replace("&mdash;", "-")
    
    # Add newline before opening block tags and after closing block tags
    block_tags = ["h2", "h3", "h4", "p", "ul", "ol", "figure", "blockquote", "table"]
    for tag in block_tags:
        html = html.replace(f"<{tag}>", f"\n<{tag}>")
        html = html.replace(f"<{tag} ", f"\n<{tag} ")
        html = html.replace(f"</{tag}>", f"</{tag}>\n")
    # Handle <li> specially — add newline before each <li> and indent with 2 spaces
...
- Virsrakstos, tekstā un sarakstos izmanto tikai "-" (nevis "—", "–", "#")
...
- In headings, text, and lists, use only "-" (never "—", "–", "#")
...
def main():
    ...
    # ── Image ────────────────────────────────────────────────────────────
    # Use Picsum with day-based seed — guarantees 365 unique images, never repeats
    # Each day gets its own deterministic unique photo
    image_src = f"https://picsum.photos/seed/day{topic_info['day']}/1200/628"
    image_alt = lv_content.get("imageQuery", "SEO digitālais mārketings")

    # ── Format content & strip forbidden characters ──────────────────────
    # Clean titles and meta descriptions of em dashes
    for content in [lv_content] + ([en_content] if en_content else []):
        content["title"] = content["title"].replace("\u2014", "-").replace("&mdash;", "-")
        content["metaDescription"] = content["metaDescription"].replace("\u2014", "-").replace("&mdash;", "-")
    # Format HTML content with proper line breaks and strip em dashes
